...
class SQL {
    ...
    async stream(t) {
        ...
        try {
            let header = await this.knex.withSchema(t.schema).from(t.table).columnInfo();
            if(process.env.DROP_COLUMNS) {
                header = dropKey(header, process.env.DROP_COLUMNS.split(','));
            }
            const csv = createCsvStringifier({header: Object.keys(header), fieldDelimiter: process.env.FIELD_DELIMITER || ','});
            const toCsv = new Transform({
                objectMode: true,
                transform(chunk, encoding, callback) {
                    const dateTypes = ['date', 'datetime', 'smalldatetime'];

                    if(process.env.DROP_COLUMNS) {
                        chunk = dropKey(chunk, process.env.DROP_COLUMNS.split(','));
                    }
